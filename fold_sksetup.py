...
# "germancredit",
    ...
    for dataset in pbar:
        ...
        df = pd.read_csv(r'./output/slack_ext/SVM/' + dataset + ".csv")
        ...
        recall_df.transpose().to_csv("./sk_data/slack_ext/SVM/" + dataset + "_recall+_.csv", header = None, index=True, sep=' ')

        prec_df = pd.DataFrame(reformed_predict)
        prec_df.columns = ['_'.join(map(str, x)) for x in prec_df.columns]
        prec_df.transpose().to_csv("./sk_data/slack_ext/SVM/" + dataset + "_prec+_.csv", header = None, index=True, sep=' ')

        acc_df = pd.DataFrame(reformed_accdict)
        acc_df.columns = ['_'.join(map(str, x)) for x in acc_df.columns]
        acc_df.transpose().to_csv("./sk_data/slack_ext/SVM/" + dataset + "_acc+_.csv", header = None, index=True, sep=' ')

        F1_df = pd.DataFrame(reformed_F1dict)
        F1_df.columns = ['_'.join(map(str, x)) for x in F1_df.columns]
        F1_df.transpose().to_csv("./sk_data/slack_ext/SVM/" + dataset + "_F1+_.csv", header = None, index=True, sep=' ')

        AOD_df = pd.DataFrame(reformed_AODdict)
        AOD_df.columns = ['_'.join(map(str, x)) for x in AOD_df.columns]
        AOD_df.transpose().to_csv("./sk_data/slack_ext/SVM/" + dataset + "_AOD-_.csv", header = None, index=True, sep=' ')

        EOD_df = pd.DataFrame(reformed_EODdict)
        EOD_df.columns = ['_'.join(map(str, x)) for x in EOD_df.columns]
        EOD_df.transpose().to_csv("./sk_data/slack_ext/SVM/" + dataset + "_EOD-_.csv", header = None, index=True, sep=' ')

        SPD_df = pd.DataFrame(reformed_SPDdict)
        SPD_df.columns = ['_'.join(map(str, x)) for x in SPD_df.columns]
        SPD_df.transpose().to_csv("./sk_data/slack_ext/SVM/" + dataset + "_SPD-_.csv", header = None, index=True, sep=' ')

        FA0_df = pd.DataFrame(reformed_FA0dict)
        FA0_df.columns = ['_'.join(map(str, x)) for x in FA0_df.columns]
        FA0_df.transpose().to_csv("./sk_data/slack_ext/SVM/" + dataset + "_FA0-_.csv", header = None, index=True, sep=' ')

        FA1_df = pd.DataFrame(reformed_FA1dict)
        FA1_df.columns = ['_'.join(map(str, x)) for x in FA1_df.columns]
        FA1_df.transpose().to_csv("./sk_data/slack_ext/SVM/" + dataset + "_FA1-_.csv", header = None, index=True, sep=' ')

        DI_df = pd.DataFrame(reformed_DIdict)
        DI_df.columns = ['_'.join(map(str, x)) for x in DI_df.columns]
        DI_df.transpose().to_csv("./sk_data/slack_ext/SVM/" + dataset + "_DI-_.csv", header = None, index=True, sep=' ')

        # flip_df = pd.DataFrame(reformed_FLIPdict)
        # flip_df.columns = ['_'.join(map(str, x)) for x in flip_df.columns]
        # flip_df.transpose().to_csv("./sk_data/slack_ext/SVM/" + dataset + "_flip_rate_.csv", header = None, index=True, sep=' ')
